...
@NamedQueries({
		@NamedQuery(name = "Ville.findAllAeroport", query = "select distinct v from Ville v left join fetch v.villeAeroports va left join fetch va.key.aeroport"),
// attention jpql n'aime pas cette ecriture de requete multiple
		@NamedQuery(name = "Ville.findAllAeroportByNom", query = "select distinct v from Ville v left join fetch v.villeAeroports va left join fetch va.key.aeroport where v.nom=:nom") })
public class Ville {
